...
Ale spróbuj poprosić go o _uruchomienie regresji wielomianowej na 24 miesiącach danych sprzedażowych i prognozę na następne 6 miesięcy._ Nagle twój agent oparty na chacie uderza w ścianę. Nie może zrobić `import sklearn`. Nie może wykonać Pythona. Może rozumować o tym, jakiego modelu użyć, ale nie może go faktycznie uruchomić.

Oczywista poprawka: daj mu środowisko Pythona. Ale jest pytanie, o którym nikt nie mówi - czy to środowisko powinno być _domyślnym runtimem_ agenta (jak Claude Code), czy raczej _narzędziem, które agent wybiera, kiedy chce_?
...
- Podejście **delegacji do sub-agenta** utrzymuje głównego agenta w czystości - opisuje _co_ przewidzieć, a sub-agent wymyśla _jak_ napisać kod Pythona.
...
Agent decyduje _kiedy_ użyć Dockera. Jeśli zapytasz "pokaż mi łączną sprzedaż według produktu", wywołuje `query_data` - szybko, bez narzutu. Jeśli zapytasz "przewidź sprzedaż Widget Alpha na następne 6 miesięcy", wywołuje `predict` - który uruchamia sub-agenta wewnątrz Dockera.
...
Agent żyje poza Dockerem. Ma narzędzie `predict`, które deleguje do sub-agenta wewnątrz Dockera. Agent decyduje _kiedy_ go użyć.

**Opcja B: Domyślne środowisko** (jak Claude Code)
Agent żyje _wewnątrz_ Dockera. Każda komenda, którą uruchamia, każdy plik, który czyta - wszystko jest w sandboxie. Środowisko jest zawsze dostępne.
...
Ale dla agenta kodującego jak Claude Code, Opcja B ma sens - całym zadaniem agenta jest czytanie, pisanie i wykonywanie kodu. Środowisko _jest_ produktem.
...
- **Delegacja do sub-agenta** utrzymuje głównego agenta w czystości. Główny agent opisuje _co_ przewidzieć. Sub-agent wymyśla _jak_ napisać kod Pythona.
