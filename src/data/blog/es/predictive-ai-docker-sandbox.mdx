...
description: "Como construir un agente IA que ejecuta predicciones sklearn en un sandbox Docker aislado. Patron environment-as-a-tool con Pydantic AI."
pubDate: 2026-02-24
...
Pero intenta pedirle que _ejecute una regresión polinómica sobre 24 meses de datos de ventas y pronostique los próximos 6 meses._ De repente tu agente basado en chat choca contra un muro. No puede hacer `import sklearn`. No puede ejecutar Python. Puede razonar sobre qué modelo usar, pero no puede ejecutar uno realmente.

La solución obvia: dale un entorno Python. Pero aquí está la pregunta de la que nadie habla - ¿ese entorno debería ser el _runtime por defecto_ del agente (como Claude Code), o debería ser _una herramienta que el agente elige usar_?
...
- El enfoque de **delegación a sub-agentes** mantiene limpio al agente principal - describe _qué_ predecir, y el sub-agente determina _cómo_ escribir el código Python.
...
El agente decide _cuándo_ usar Docker. Si preguntas "muéstrame las ventas totales por producto", llama a `query_data` - rápido, sin overhead. Si preguntas "predice las ventas de Widget Alpha para los próximos 6 meses", llama a `predict` - que lanza un sub-agente dentro de Docker.
...
El agente vive fuera de Docker. Tiene una herramienta `predict` que delega a un sub-agente dentro de Docker. El agente decide _cuándo_ usarlo.

**Opción B: Entorno por Defecto** (como Claude Code)
El agente vive _dentro_ de Docker. Cada comando que ejecuta, cada archivo que lee - todo está en el sandbox. El entorno siempre está ahí.
...
Pero para un agente de codificación como Claude Code, la Opción B tiene sentido - toda la tarea del agente es leer, escribir y ejecutar código. El entorno _es_ el producto.
...
- **Delegación a sub-agentes** mantiene limpio al agente principal. El agente principal describe _qué_ predecir. El sub-agente determina _cómo_ escribir el código Python.
