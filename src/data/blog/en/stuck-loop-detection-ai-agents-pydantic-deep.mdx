...
<div className="my-6 grid gap-3 sm:grid-cols-3">
  <Stat value="47" label="identical tool calls" />
  <Stat value="$12" label="wasted on retries" />
  <Stat value="0" label="forward progress" />
</div>
...
<Callout type="warn" title="Why this matters at scale">
  A 47-call loop overnight on one task is annoying. The same pattern across multiple concurrent
  agents in production is how API bills triple in a single weekend.
</Callout>
