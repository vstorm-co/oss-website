...
seoTitle: "AI Agent App Web Configurator: 75+ Options"
description: "Web configurator for AI agent apps: 246 templates, 5 AI frameworks, client-side Nunjucks rendering. 75+ options, download as ZIP — zero server needed."
pubDate: 2026-03-06
...
> **TL;DR:** The web configurator for the full-stack-ai-agent-template provides a 9-step visual wizard that configures 75+ options -- database, auth, AI framework (Pydantic AI, LangChain, LangGraph, CrewAI, DeepAgents), infrastructure, observability, CI/CD, and frontend -- and generates a production-ready project as a ZIP file entirely client-side with zero server involved. The full pipeline of fetching 246 bundled templates, rendering them with Nunjucks (a browser-compatible Jinja2 engine), excluding unused files, and generating a DEFLATE-compressed ZIP takes 1-2 seconds. Three presets (minimal, production, AI agent) let users go from landing page to downloaded project in under 10 seconds. The auto-fix dependency system silently enables Redis when caching is turned on and disables Logfire database instrumentation when no database is selected, ensuring the user never sees an invalid configuration state. Zod validation mirrors the Python CLI's Pydantic models with the same constraints: 14 enum types, 28 boolean toggles, and 12 visibility functions for cross-field conditional rendering.

CLIs are powerful. But they're also intimidating.

Our full-stack-ai-agent-template — which we think of as the [create-ai-app moment for AI applications](/blog/create-ai-app-new-default/) — has 75+ configuration options. Database, auth, AI framework, background tasks, caching, rate limiting, observability, Docker, Kubernetes, CI/CD, frontend, i18n — the list goes on. The CLI works great for developers who already know what they want. For everyone else, it's a wall of flags and prompts.
...
For a detailed guide on what the generated project includes, see [how to build a full-stack AI app with FastAPI and Next.js](/blog/fastapi-nextjs-ai-template-guide/), or jump straight to [shipping a production AI app fast](/blog/ship-production-ai-app-fast/).
