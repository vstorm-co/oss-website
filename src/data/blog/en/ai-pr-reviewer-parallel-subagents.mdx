...
seoTitle: "AI PR Reviewer with 3 Parallel Subagents"
...
> **TL;DR:** An AI PR reviewer built with pydantic-deepagents uses 3 parallel subagents -- security, style, and performance -- to produce a structured, priority-sorted code review in approximately 30 seconds, compared to roughly 90 seconds for sequential execution. The entire implementation is under 40 lines of actual logic using `create_deep_agent()` with three `SubAgentConfig` definitions. Each subagent receives the same git diff and reviews through its specialized lens: the security reviewer checks for SQL injection, XSS, and hardcoded secrets; the style reviewer catches naming violations, missing type hints, and dead code; the performance reviewer flags N+1 queries, blocking I/O in async contexts, and unoptimized loops. The parent coordinator aggregates, deduplicates, and sorts findings by severity -- critical first, then warnings, then informational. This deep agent pattern of plan-delegate-execute-synthesize is the same architecture used by Claude Code and OpenAI Codex, but open-source and built on Pydantic AI.
...
This is the deep agent pattern: a coordinator that plans, delegates to specialists, and synthesizes results. It's the same architecture used by Claude Code, Codex, and other production coding agents — but open-source and built on [Pydantic AI](https://ai.pydantic.dev/). We explored this architecture in depth in our post on [building an open-source Claude Code alternative](/blog/open-source-claude-code-alternative/).
...
- **`description`** — tells the parent _when_ to use this subagent (the routing logic)
...
The pattern — which we break down further in [task planning for AI agents](/blog/task-planning-ai-agents/):
...
- **Subagents package:** [subagents-pydantic-ai](/projects/subagents-pydantic-ai/)
...
_I'm Kacper, AI Engineer at [Vstorm](https://vstorm.co) — an Applied Agentic AI Engineering Consultancy. We build and open-source production AI agent tooling in Python. Star [pydantic-deepagents](https://github.com/vstorm.co/projects/pydantic-deepagents) on GitHub if you find it useful._
