...
<TLDR>
  pydantic-deep **v0.3.5** ships `BrowserCapability` — 9 Playwright tools (navigate, click,
  type_text, get_text, screenshot, scroll, go_back, go_forward, execute_js) with domain allowlist,
  single-tab design, and automatic popup interception. **v0.3.4** redesigns `/improve`: extracts
  `UserFactInsight` and `AgentLearningInsight` from **raw tool traces** (not summaries) into
  MEMORY.md. Key finding: raw traces significantly outperform summaries for producing actionable
  agent learnings.
</TLDR>
...
This week I shipped the fix in pydantic-deep v0.3.4, along with browser automation in v0.3.5. Combined with [StuckLoopDetection](/blog/stuck-loop-detection-ai-agents-pydantic-deep/), [context window warnings](/blog/context-window-blindness-ai-agents-limit-warner/), [curl install](/blog/pydantic-deep-one-command-install-curl-bash/), and [Docker sandbox](/blog/ai-agents-docker-sandbox-ci-cd-pydantic-deep/) shipped earlier this week, the two together changed what an AI agent actually is for me.
...
2. Agent knows when context is running out ([LimitWarnerCapability](/blog/context-window-blindness-ai-agents-limit-warner/))
