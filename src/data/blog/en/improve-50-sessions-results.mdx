...
import MetricsBar from "@/components/blog/MetricsBar.astro";
import DeploymentStat from "@/components/blog/DeploymentStat.astro";
...
<MetricsBar
  metrics={[
    { value: "50", label: "Real Sessions" },
    { value: "180+", label: "Insights Extracted" },
    { value: "3,200", label: "Messages Analyzed" },
    { value: "7", label: "Insight Types" },
  ]}
/>

<TLDR>
  We ran pydantic-deep's `/improve` pipeline on 50 real development sessions (~3,200 messages, 7
  days) and extracted **180+ structured insights** across 7 types. AgentLearning insights (build
  commands, file paths, tool chains) dominated at 45%, followed by Pattern (20%) and Failure (15%).
  The pipeline proposed 18 changes to context files with calibrated confidence scores, MEMORY.md
  went from empty to a structured knowledge base. The most valuable single insight prevented a build
  command failure that had repeated across **6 separate sessions**.
</TLDR>
...
<DeploymentStat
  value="6"
  unit="separate sessions"
  finding="hit the same `pytest` vs `uv run pytest` build command failure before /improve captured it as a single AgentLearningInsight in MEMORY.md"
  context="50 real pydantic-deep development sessions"
  source="Session log analysis, Q1 2026"
/>
...
<RepoCard
  ...
  description="Production-ready Python framework for autonomous AI agents with Claude-Code-style architecture. Includes /improve pipeline for self-improving agents."
  language="Python"
/>
