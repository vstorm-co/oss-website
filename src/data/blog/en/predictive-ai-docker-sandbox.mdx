...
The obvious fix: give it a Python environment. But here's the question nobody talks about - should that environment be the agent's *default runtime* (like Claude Code), or should it be *a tool the agent chooses to use*? For cloud-hosted sandboxes with sub-90ms startup, see our post on [Daytona replacing Docker](/blog/daytona-sub-90ms-code-execution/) in our agent stack.
...
The agent lives *inside* Docker. Every command it runs, every file it reads - it's all in the sandbox. The environment is always there.
