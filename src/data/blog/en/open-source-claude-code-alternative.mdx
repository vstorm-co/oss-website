...
Each package works standalone. You can use [`pydantic-ai-todo`](https://github.com/vstorm-co/pydantic-ai-todo) without the rest. You can swap [`pydantic-ai-backend`](https://github.com/vstorm-co/pydantic-ai-backend) for your own storage layer. The framework composes them — you pick what you need.
...
Each team member shares a `SharedTodoList` (asyncio-safe task claiming with dependencies) and a `TeamMessageBus` for peer-to-peer messaging. No hierarchical subagent chains — agents coordinate as peers, claim tasks, and message each other when they need input.
