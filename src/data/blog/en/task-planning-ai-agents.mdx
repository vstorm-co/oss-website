...
seoTitle: "Task Planning for AI Agents: Deps & Events"
...
> **TL;DR:** Without structured task planning, AI agents skip steps, repeat work, and lose track of progress -- asking an agent to "build a REST API with authentication" results in it jumping to code before database design, forgetting migrations, and implementing auth middleware before the user model exists. The pydantic-ai-todo package provides a standalone planning toolset for Pydantic AI with in-memory, async memory, and PostgreSQL backends. Agents get tools to create, track, and manage structured tasks with statuses (pending, in_progress, completed, blocked), hierarchical subtasks, and dependency chains where "auth system needs user model" becomes a hard constraint enforced by the system. Cycle detection via depth-first search prevents deadlocks from circular dependencies. The PostgreSQL backend supports multi-tenant production deployments with session-scoped storage, connection pooling, and auto-schema creation. An event system fires notifications for task creation, completion, and status changes, enabling dashboards and audit logging. The package integrates via a single line: `create_todo_toolset()` added to the agent's toolsets.
...
We've seen this pattern kill agent reliability at Vstorm. The fix is surprisingly simple: give the agent a todo list. Not a metaphorical one - an actual tool that creates, tracks, and manages structured tasks with statuses, subtasks, and dependencies. This is a core component of our [open-source Claude Code alternative](/blog/open-source-claude-code-alternative/).
...
- **Structured planning reduces hallucinated steps.** Without a todo list, agents skip steps, repeat work, and lose track of progress. With one, they plan systematically and execute in order. Combine this with proper [memory management](/blog/ai-agent-memory-management/) to ensure long-running agents don't forget their progress.
...
[pydantic-ai-todo](/projects/pydantic-ai-todo/) - Task planning toolset for Pydantic AI agents with subtasks, dependencies, events, and PostgreSQL multi-tenant support.
