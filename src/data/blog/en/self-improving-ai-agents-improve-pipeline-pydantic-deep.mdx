...
Session ends
    ↓
tool_log.jsonl saved (raw tool calls, up to 20K chars)
    ↓
/improve command triggered
    ↓
Extraction agent reads raw tool traces
    ↓
Extracts: UserFactInsight + AgentLearningInsight
    ↓
Synthesis agent: insights + raw traces → structured improvements
    ↓
MEMORY.md updated
    ↓
Next session: agent loads MEMORY.md on startup
```
