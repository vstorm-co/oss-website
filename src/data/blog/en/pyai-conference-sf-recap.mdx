---
title: "PyAI Conf SF Recap: Where Python Meets Production AI Agents"
seoTitle: "PyAI Conf SF: Where Python Meets AI Agents"
description: "Same-day recap from PyAI Conf — talks by Samuel Colvin, Guido van Rossum, hallway conversations, and what it means for production AI agents."
pubDate: 2026-03-10
...
> **TL;DR:** PyAI Conf in San Francisco brought together Samuel Colvin (Pydantic), Sebastian Ramirez Montano (FastAPI), Jeremiah Lowin (Prefect/FastMCP), Armin Ronacher, and Guido van Rossum for a one-day conference hosted by Pydantic, FastMCP, and Theory Ventures. The key panel takeaways: Python's role in AI is intentional, not accidental, but the language needs better async primitives and performance for long-running agents; AI coding agents are reshaping open source maintenance as agent-generated PRs look fundamentally different; and type safety in LLM interactions is what separates prototype from production code. Hallway conversations revealed convergence across frameworks -- teams using Pydantic AI, LangChain, LangGraph, and CrewAI are all arriving at the same patterns (typed tools, structured outputs, sub-agent delegation) despite different APIs. The consensus on multi-agent: bounded sub-agent delegation with 2-3 specialists works in production, while "swarms of 15 autonomous agents" remains aspirational. The shift in community conversation was from "can your framework do X?" to "what happens when X fails at 3am?"

_One day. The creators of Python, Pydantic, FastAPI, and FastMCP in one room. Here's what happened._
...
**Type safety is the foundation, not the feature.** Samuel made the case that type safety in LLM interactions isn't a nice-to-have — it's what makes the difference between prototype code and production code. When your agent returns validated, typed data, you can build reliable systems on top of it. When it returns `Any`, you're writing parsing code forever. This aligns with what we've seen in our own [Pydantic AI vs LangChain](/blog/pydantic-ai-vs-langchain/) comparison — type safety is the single biggest differentiator.
...
**Framework convergence is real.** I talked to engineers using different frameworks — [Pydantic AI](https://ai.pydantic.dev/), LangChain, LangGraph, CrewAI — and they're all converging on the same patterns: typed tool definitions, structured outputs, pluggable model providers, sub-agent delegation. The APIs look different, but the architecture underneath is aligning. We documented this convergence in our [framework comparison guide](/blog/choosing-ai-framework/).
...
**The "plan, execute, delegate" pattern keeps showing up.** Multiple people I talked to — independently of each other — described the same architecture for production agents: an LLM that plans, tools that execute, sub-agents that specialize. It's the pattern behind Claude Code, and it's what we implement in our [open-source Claude Code alternative](/blog/open-source-claude-code-alternative/) using [pydantic-deepagents](https://github.com/vstorm.co/projects/pydantic-deepagents). When multiple teams converge on the same architecture from different directions, that's a signal.
...
- [Pydantic AI documentation](https://ai.pydantic.dev/) — The framework discussed throughout the conference
- [FastAPI documentation](https://fastapi.tiangolo.com/) — Sebastian's framework, the backbone of most Python AI backends
