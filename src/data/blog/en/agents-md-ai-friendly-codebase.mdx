...
seoTitle: "AGENTS.md: AI-Agent Friendly Codebase Guide"
...
> **TL;DR:** AGENTS.md is an emerging tool-agnostic standard for making codebases AI-agent friendly. While every major AI coding tool reads its own config file -- GitHub Copilot uses `.github/copilot-instructions.md`, Cursor uses `.cursor/rules/`, Claude Code reads `CLAUDE.md`, and OpenAI Codex reads `AGENTS.md` -- the AGENTS.md format is converging as the single file that works across at least 5 tools including Copilot, Codex, Zed, Claude Code, and Cursor. The file takes about 15 minutes to write and contains project overview, commands, structure, conventions, and off-limits sections. The most impactful sections are Commands (preventing wrong build tools) and Conventions (preventing wrong architectural patterns). Auto-generation is also possible: the full-stack-ai-agent-template generates both AGENTS.md and CLAUDE.md tailored to each project's exact configuration from 75+ options, so AI coding agents get project-specific context without manual effort.
...
Every major AI coding tool — [GitHub Copilot](https://github.com/features/copilot), [Cursor](https://cursor.sh/), [OpenAI Codex](https://openai.com/index/codex/), [Claude Code](https://docs.anthropic.com/en/docs/claude-code) — can now read, understand, and modify entire repositories. We explored this architecture in depth in our post on [building an open-source Claude Code alternative](/blog/open-source-claude-code-alternative/). They're not autocompleting single lines anymore. They're refactoring modules, writing tests, creating new features from natural language descriptions.
...
- **Auto-generation is possible** — our template generates AGENTS.md and CLAUDE.md tailored to each project's exact configuration. See our [framework comparison guide](/blog/choosing-ai-framework/) for how different tools use these files.
