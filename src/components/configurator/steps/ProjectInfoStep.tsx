...
export function ProjectInfoStep() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-text text-xl font-semibold tracking-tight">Project Info</h2>
        <p className="text-text-tertiary mt-1 text-sm">Basic project details</p>
      </div>
        ...
        <Field label="Project Name" error={errors.project_name?.message} required>
          ...
          {showNormalized ? (
            <p className="mt-1 text-[11px] text-amber-400/80">
              Will be saved as: <code className="font-mono text-amber-300">{normalized}</code>
            </p>
          ) : (
