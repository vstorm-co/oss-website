...
const architectures: Architecture[] = [
  {
    ...
    description:
      "A single agent that reasons and acts in a loop — picks a tool, observes the result, and decides next steps.",
  ...
  },
  {
    ...
    description: "An enhanced ReAct agent with multiple tools and session memory.",
  ...
  },
  {
    id: "pipeline",
    name: "Pipeline Agent",
    description: "A sequential processing chain where each step transforms or enriches the data.",
  ...
  },
  {
    ...
    name: "RAG Pipeline",
    description:
      "Retrieval-Augmented Generation — queries a knowledge base to ground the LLM's response in real data.",
    when: "Knowledge bases, documentation search, domain-specific Q&A.",
    framework: "Pydantic AI",
    frameworkSlug: "pydantic-ai",
  ...
  },
  {
    ...
    description: "Multiple specialized agents that collaborate on a task.",
    when: "Complex tasks requiring different skills — research + writing, analysis + visualization.",
    framework: "CrewAI",
  ...
  },
  {
    ...
    description:
      "An orchestrator agent that plans, delegates sub-tasks to specialist agents, reviews their work, and synthesizes results.",
    when: "Enterprise workflows, deep research, complex automation.",
    ...
    <div>
      ...
      <div className="mb-8">
        <div className="text-text-secondary mb-2 flex items-center justify-between text-sm">
          <span>
            {showResult ? "Result" : `Question ${currentStep + 1} of ${questions.length}`}
          </span>
          {(currentStep > 0 || showResult) && (
            <button
              onClick={goBack}
              className="text-violet-600 transition-colors hover:text-violet-500"
            >
              ← Back
            </button>
          )}
      ...
      {!showResult && question && (
        <div className="space-y-4">
          <h3 className="text-text text-xl font-semibold">{question.label}</h3>
          <div className="space-y-3">
            {question.options.map((option, i) => {
              const isSelected =
                answers[question.id] &&
                JSON.stringify(answers[question.id]) === JSON.stringify(option.traits);
              return (
                <button
                  key={i}
      ...
      {showResult && result && (
        <div className="space-y-6">
          <div className="rounded-xl border border-violet-500/30 bg-violet-50 p-6 sm:p-8">
            ...
            <h3 className="text-text text-2xl font-bold">{result.name}</h3>
            <p className="text-text-secondary mt-3 leading-relaxed">{result.description}</p>

            <div className="border-border bg-surface mt-6 flex flex-wrap items-center gap-2 rounded-lg border p-4">
                ...
                <span
                  ...
                  className={
                    node === "→" || node === "⟲" || node === "+"
                      ...
                      : "border-border bg-bg text-text-secondary rounded-lg border px-3 py-1 font-mono text-sm"
                  }
                >
                  {node}
                </span>
            ...
            <div className="border-border bg-surface mt-6 rounded-lg border p-4">
              <p className="text-text-secondary text-sm">
                <strong className="text-text">Best for:</strong> {result.when}
              </p>
            </div>
            <div className="border-border bg-surface mt-4 rounded-lg border p-4">
              <p className="text-text-secondary text-sm">
                <strong className="text-text">Recommended framework:</strong>{" "}
                <span className="text-violet-700">{result.framework}</span>
              </p>
            </div>
          </div>
